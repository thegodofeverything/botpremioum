...
module.exports = msgHandler = async (client, message) => {
	...
	if (isGroupMsg && isRule && (type === 'chat' && message.body.includes('chat.whatsapp.com') && isBotGroupAdmins) && !isGroupAdmins) return await client.removeParticipant(chat.id, author)
        ...
        switch (command) {
            case 'sticker':
			case 'st':
					let metadata = { author: "✒ вιиα ℓσνєѕ яαנєи ₶", pack: "💯𝚠𝚑𝚎𝚗 𝚔𝚊𝚔𝚊 𝚠𝚊𝚜 𝚢𝚘𝚞𝚗𝚐 ܊" };
					...
					if (
						...
						quotedMsgObj.mimetype !== "image/gif")
							...
							client.reply(chatId, "Wait for some time Senpai!", id);
							...
							try {
								...
								await client.reply(
									...
									: "Some error occurred, gomen nasai   :(",
									id
								);
							}
       ...
       case 'stickermeme':
	   case 'stcmeme':
		if (!q.includes('|')) return await client.reply(from, 'Incorrect format, Baka', id)
                if (isMedia && type === "image" || quotedMsg && quotedMsg.type === "image") {
                    await client.reply(from, 'Wait Senpai !', id);
                    ...
                    await client.sendStickerfromUrl(from, create, null, { author: '✒ вιиα ℓσνєѕ яαנєи ₶', pack: '*⋆  🎀 𝖇𝖔𝖙🎀  ⋆*', keepScale: true });
                } else {
                    await client.reply(from, 'Incorrect format, Baka!', id);
                }
			break
        ...
        case 'doge':
                fun.doge()
                    .then(async (body) => {
                        ...
                        await client.sendStickerfromUrl(from, dogegx, null, { author: '✒ вιиα ℓσνєѕ яαנєи ₶', pack: '💯𝚠𝚑𝚎𝚗 𝚔𝚊𝚔𝚊 𝚠𝚊𝚜 𝚢𝚘𝚞𝚗𝚐 ܊' })
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await client.reply(from, 'Gomen nasai Senpai, something went wrong!!', id)
                    })
            break
        ...
        case 'wh':
                fun.wholesome()
                    .then(async (body) => {
                        ...
                        await client.sendStickerfromUrl(from, wholesomegx, null, { author: '✒ вιиα ℓσνєѕ яαנєи ₶ ', pack: '💯𝚠𝚑𝚎𝚗 𝚔𝚊𝚔𝚊 𝚠𝚊𝚜 𝚢𝚘𝚞𝚗𝚐 ܊' })
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await client.reply(from, 'Gomen nasai Senpai, something went wrong!!', id)
		...
		case 'animesticker': // by ANIME API HEROKU 9876Code Sendo
        case 'astick':
                fun.animestick()
                    .then(async (body) => {
                        const animestickg = body.split('\n')
                        const animestickgx = animestickg[Math.floor(Math.random() * animestickg.length)]
                        await client.sendStickerfromUrl(from, animestickgx, null, { author: '✒ вιиα ℓσνєѕ яαנєи ₶ ', pack: '💯𝚠𝚑𝚎𝚗 𝚔𝚊𝚔𝚊 𝚠𝚊𝚜 𝚢𝚘𝚞𝚗𝚐 ܊' })
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await client.reply(from, 'Gomen nasai Senpai, something went wrong!!', id)
	   ...
	   case 'ocr':
		...
		case 'dare':
                fun.dare()
                    ...
                    .catch(async (err) => {
                        console.error(err)
                        await client.reply(from, 'Gomen Nasai, something went wrong!!', id)
                    })
            break
       ...
       case 'quotemaker':
		...
		case 'aiquote' :
            ...
            await client.sendFileFromUrl(from, aiquote.data, 'quote.jpg', 'Powered By BinaBOT🔮' , id )
            break
...
${desc}`)
        ...
        case 'bc':
            if(!isowner) return client.reply(from, 'This command is for Satyam Only!', message.id)
            let msg = body.slice(4)
            const chatz = await client.getAllChatIds()
            for (let ids of chatz) {
                var cvk = await client.getChatById(ids)
                if (!cvk.isReadOnly) client.sendText(ids, `[ ✒ вιиα ℓσνєѕ яαנєи ₶ BOT Broadcast ]\n\n${msg}`)
            }
        ...
        case 'boycott':
        case 'ban':
            if(!isowner) return client.reply(from, 'Only Satyam can use this CMD!', message.id)
        ...
        case 'ping':
            if (!isGroupMsg) return client.reply(from, 'Gomen Nasai, This command can only be used in groups', message.id)
        ...
        case 'kickall':
            if(!isowner) return client.reply(from, 'Gomen Nasai only Satyam can use this CMD', message.id)
		...
		try {
			...
			const isnsfw = nsfwgrp.includes(from)
                if (nsfw == true) {
                      if ((isGroupMsg) && (isnsfw)) {
                                ...
                                await client.reply(from, `!Warning Adult content ! NSFW is not registered on *${name}*`, id)
                      }
                 else {
                      await client.sendFileFromUrl(from, `${url}`, 'Husbando.jpg', `${title}` + '\n\nPostlink:' + `${postLink}`)
                }
            ...
            return client.reply(from, 'Gomen nasai ! Failed to get image, Try again later', id)
        }
		break

		case 'clearall':
            if (!isowner) return client.reply(from, 'Satyam can only use this CMD', message.id)
        ...
        case 'act':
             ...
             if (!isGroupAdmins) return client.reply(from, 'Only Admins can use this command, Baka >.<', id)
				...
				client.reply(from, `!Warning Adult content! NSFW is already registered on *${name}*`, message.id)
	     			} else {
                		nsfwgrp.push(chat.id)
                		fs.writeFileSync('./lib/nsfwg.json', JSON.stringify(nsfwgrp))
                		client.reply(from, `!Warning Adult Content! NSFW is now registered on *${name}*`, message.id)
				}
       ...
       case 'cgc':
        ...
        case 'snap':
            if(!isGroupMsg) return client.reply(from, '...', message.id)
            if(!isGroupAdmins) return client.reply(from, 'You are not an admin, Gomen nasai', message.id)
            if(!isBotGroupAdmins) return client.reply(from, 'You need to make me admin to use this CMD', message.id)
        ...
        case 'join':
            ...
            if (check.size < minMem) return client.reply(from, 'The group does not have 2+ members', message.id)
            await client.joinGroupViaLink(link).then( async () => {
                await client.reply(from, '*Joined* ✨️', message.id)
            }).catch(error => {
                client.reply(from, 'An error occured 💔️ gomen nasai', message.id)
            })
            break
        ...
        case 'setstats':
        case 'setstat':
                if (!isowner) return await client.reply(from, 'gomen nasai Senpai, Only Satyam can do this!', id)
...
🌐️ *URL*: ${url}`
            ...
            client.sendImage(from, base64, title, content)
             ...
             await client.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Gomen nasai, Couldn\'t find the requested anime')
           }
          break
	...
	case 'manga':
           ...
           `https://api.jikan.moe/v3/search/manga?q=${keywrd}`
            ...
            const parsed = await data.json()
            if (!parsed) {
              await client.sendFileFromUrl(from, errorurl2, 'error.png', 'Gomen nasai, Couldn\'t find the requested manga', id)
              console.log("Sent!")
              return null
              }
...
🌐️*Link*: ${url}`
            ...
            client.sendImage(from, base64, title, content)
             ...
             console.error(err.message)
             await client.sendFileFromUrl(from, errorurl2, 'error.png', 'Gomen nasai!, Couldn\'t find the requested manga')
           }
          break
...
*Link*: ${url}`
		...
		case 'gecg':
			await client.sendFileFromUrl(from, (await neko.sfw.gecg()).url, 'gecg.jpg', '', null, null, true)
                    ...
                    .catch(async (err) => {
                        console.error(err)
                        await client.reply(from, 'Goemn nasai Senpai, something went wrong!', id)
                    })
            break
		...
		case 'slap':
			client.sendStickerfromUrl(from, (await neko.sfw.slap()).url, null, { author: '✒ вιиα ℓσνєѕ яαנєи ₶', pack: '💯𝚠𝚑𝚎𝚗 𝚔𝚊𝚔𝚊 𝚠𝚊𝚜 𝚢𝚘𝚞𝚗𝚐 ܊', keepScale: true });
        ...
        case 'slap':
            ...
            await client.sendGiphyAsSticker(from, 'https://media.giphy.com/media/Qv7WFppXtkqkM/giphy.gif')
	...
	//end of nsfw commands
		...
		case 'nsfw':
            ...
            return client.reply(from, 'Gomen Nasai! senpai!, Try again later', id)
        }
            break
